// src/components/InteractiveStarRating.tsx
'use client';

import React, { useState } from 'react';

interface InteractiveStarRatingProps {
  onRate: (rating: number) => void; // Функция вызывается при выборе рейтинга
  currentRating?: number; // Текущий рейтинг (если есть)
  size?: 'sm' | 'md' | 'lg';
  disabled?: boolean; // Отключить интерактивность
}

const InteractiveStarRating: React.FC<InteractiveStarRatingProps> = ({ 
  onRate, 
  currentRating = 0,
  size = 'md',
  disabled = false
}) => {
  const [hoverRating, setHoverRating] = useState(0);
  const [selectedRating, setSelectedRating] = useState(currentRating);

  // Размеры звездочек
  const starSize = {
    sm: 'w-4 h-4',
    md: 'w-6 h-6', 
    lg: 'w-8 h-8'
  }[size];

  const handleStarClick = (rating: number) => {
    if (disabled) return;
    setSelectedRating(rating);
    onRate(rating);
  };

  const handleStarHover = (rating: number) => {
    if (disabled) return;
    setHoverRating(rating);
  };

  const handleMouseLeave = () => {
    if (disabled) return;
    setHoverRating(0);
  };

  const renderStar = (starNumber: number) => {
    const isActive = starNumber <= (hoverRating || selectedRating);
    
    return (
      <button
        key={starNumber}
        type="button"
        className={`${starSize} transition-colors duration-150 ${
          disabled 
            ? 'cursor-default' 
            : 'cursor-pointer hover:scale-110 transform transition-transform'
        }`}
        onClick={() => handleStarClick(starNumber)}
        onMouseEnter={() => handleStarHover(starNumber)}
        onMouseLeave={handleMouseLeave}
        disabled={disabled}
        title={`Оценка ${starNumber} из 5`}
      >
        <svg
          className={`w-full h-full transition-colors ${
            isActive 
              ? 'text-yellow-400' 
              : disabled 
                ? 'text-gray-300'
                : 'text-gray-300 hover:text-yellow-300'
          }`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
      </button>
    );
  };

  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map(renderStar)}
      {(hoverRating > 0 || selectedRating > 0) && (
        <span className="ml-2 text-sm text-gray-600">
          {hoverRating > 0 ? hoverRating : selectedRating} из 5
        </span>
      )}
    </div>
  );
};

export default InteractiveStarRating;